...
def month_anomaly_means(dataset):
...
def month_anomaly_means_ET(dataset):
    ...
    for month in months:
        ...
        ds_month_avg = dataset.isel(time=idxs).mean(dim='time')
        dataset_month_anomaly = dataset.isel(time=idxs) - ds_month_avg
        dataset_month_anomalies.append(dataset_month_anomaly.ET.mean(dim=['x','y']))
        print(str(month))
...
#et_anomaly = month_anomaly_means_ET(et)
...
ndvi_anomaly = month_anomaly(ndvi)
ndvi_anomaly_df = ndvi_anomaly.NDVI.mean(dim=['x','y']).to_dataframe()
