...
clm_r_anom = ((clm_r.mean(dim={'x','y'})) - (clm_r.mean(dim={'x','y'}).mean()))
...
#TWS
clm_tws = xr.open_mfdataset(files[-1],parallel=True).mm
clm_tws_anom = (clm_gw.mean(dim={'x','y'}) - clm_gw.mean(dim={'x','y'}).mean())
...
#for precip/runoff/ET??
def growing_annual_sum(dataset):
    dataset['water_year'] = dataset.time.dt.year.where(dataset.time.dt.month < 10, dataset.time.dt.year + 1)
    dataset = dataset[dataset.time.dt.month.isin(growing_months)]
    annual_data = [dataset.loc[dataset.water_year == i].sum(axis=0) for i in range(start_year,end_year)]
    return annual_data
...
#PPT
precip_variables = [(p_gpm.P_mm), (p_chirps.P_mm)]
[print(var.dims) for var in precip_variables]
precip_variable_names = ['PPT_GPM','PPT_CHIRPS']
[growing_anom(growing_annual_sum(var)).to_netcdf(netcdf_anom_path+r'\{}.nc'.format(name)) for var,name in zip(precip_variables,precip_variable_names)]
...
#RUNOFF
clm_r
clm_r_seasonal = growing_anom(growing_annual_sum(clm_r)) #.to_netcdf(netcdf_anom_path+r'\Runoff.nc')
...
#SOIL MOISTURE
clm_sm_rz.mean(dim={'x','y'}) 
clm_sm_surface.mean(dim={'x','y'})

#TWS
clm_tws_anom = (clm_gw.mean(dim={'x','y'}) - clm_gw.mean(dim={'x','y'}).mean())

#P Anomalies
p_anomalies = p_gpm['P_mm'].mean(dim={'x','y'}) - p_gpm['P_mm'].mean(dim={'x','y'}).mean()
#ET Anomalies
et_anomalies = et_modis['ET_kg_m2'].mean(dim={'x','y'})  - et_modis['ET_kg_m2'].mean(dim={'x','y'}).mean()
#R Anomalies
q_anomalies = clm_r.mean(dim={'x','y'}) - clm_r.mean(dim={'x','y'}).mean()
...
RHS_GPM = p_gpm['P_mm'].mean(dim={'x','y'}) - et_modis['ET_kg_m2'].mean(dim={'x','y'}) - clm_r.mean(dim={'x','y'}) - clm_sm_surface.mean(dim={'x','y'}) # - clm_sm_rz.mean(dim={'x','y'}) 
RHS_df = RHS_GPM.rename('RHS').to_dataframe()


plt.figure(figsize=(8,4))
plt.plot(grace_limpopo_new_xr.time, grace_limpopo_new_xr.mean(dim={'lat','lon'})*1000,label='GRACE TWSA',color='darkblue')
#CLSM_GW.plot(color='orange')
CLSM_GW.plot(label='CLSM GWSA')
...
def linear_plot(independent,dependent,var1,var2):
    ...
    slope, intercept, r_value, p_value, std_err = stats.linregress(independent_mask, dependent_mask)
    y = np.array(dependent_mask)
    x = np.array(independent_mask)

    fig = plt.figure(figsize=(4,4))
    ax = fig.add_subplot()
    ax.scatter(independent_mask,dependent_mask,s=4)
    ax.plot([], [], ' ', label='p-val = {}'.format(round(p_value, 4)))
    ax.plot([], [], ' ', label='r: {}'.format(round(r_value,4)))
    #ax.set_xlim(-2,2)
    #ax.set_ylim(-2,2)
    ax.legend(loc='upper left')
    ax.set_title('{} vs. {}'.format(var1,var2))

RHS_budget_anom = RHS_df.shift(periods=4, freq="M").RHS
CLSM_GW_anom = CLSM_GW
linear_plot(RHS_df.RHS[2:],CLSM_GW_anom[:-2],'RHS','CLSM GW')
linear_plot(RHS_df.RHS[1:],CLSM_GW_anom[:-1],'RHS','CLSM GW')
linear_plot(RHS_df.RHS,CLSM_GW_anom,'RHS','CLSM GW')
...
plt.figure(figsize=(8,4))
plt.plot(grace_limpopo_new_xr.time, grace_limpopo_new_xr.mean(dim={'lat','lon'})*1000,label='GRACE TWSA',color='darkblue')
#plt.plot(grace_limpopo_new_xr.time.resample({'time':'MS'}).mean()[10:-1], (grace_limpopo_new_xr.mean(dim={'lat','lon'})*1000).resample({'time':'MS'}).mean()[10:-1] - CLSM_GW.resample({'time':'MS'}).mean(),label='GRACE TWSA - CLSM GWSA',color='darkblue')
plt.plot(CLSM_GW.time, CLSM_GW - (clm_r.mean(dim={'x','y'}) + clm_sm_surface.mean(dim={'x','y'})))
plt.xlim(pd.to_datetime('2003-02-01'),pd.to_datetime('2022-07-01'))
plt.axhline(0,color='black',linestyle='--',linewidth=0.5)
plt.title('Monthly Anomalies',weight='bold',fontsize=15)
plt.xlabel('Date',weight='bold',fontsize=12)
...
plt.tight_layout()
plt.legend()
#plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\GRACE_CLSM_diff.png',dpi=300)

from scipy.stats import gaussian_kde
def linear_plot(model,insitu,title):
    try:

        insitu = (grace_limpopo_new_xr.mean(dim={'lat','lon'})*1000)[10:-1]
        model = CLSM_GW - (clm_r.mean(dim={'x','y'}) + clm_sm_surface.mean(dim={'x','y'}))
        ...
        ax.set_ylabel('Model (mm)', fontsize=13)
        ax.set_xlabel('InSitu (mm)', fontsize=13)
        ax.legend()
        #ax.set_xlim(0,600)
        #ax.set_ylim(0,600)
        #ax.plot([0,600],[0,600],'--',color='black')
        ax.text(100,-50,s="rmse: {}".format(round(float(rmse),3)), fontsize=11, ha="left", va="top")
        ax.text(100,-60,s="bias: {}".format(round(float(bias),3)), fontsize=11, ha="left", va="top")
        ax.text(100,-40,s="p-val: {}".format(round(float(p_value),3)), fontsize=11, ha="left", va="top")
...
#linear_plot(CLSM_GW,(grace_limpopo_new_xr.mean(dim={'lat','lon'})*1000)[10:-1],'RHS-CLSM GW')
linear_plot(clm_sm_rz.mean(dim={'x','y'}) - clm_sm_rz.mean(dim={'x','y'}).mean() + CLSM_GW,(grace_limpopo_new_xr.mean(dim={'lat','lon'})*1000)[10:-1],'RHS-CLSM GW')
...
files_spei = sorted(glob.glob(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\Data\climate_indices\SPEI\GPM\*.nc'))
files_indicators = [files_vhi[1],files_spei[2],files_spei[0],files_spei[1]]
indices_datasets = [xr.open_mfdataset(file,parallel=True) for file in files_indicators]
inidices_keys = [list(ds.data_vars)[0] for ds in indices_datasets]
...
ax = fig.add_subplot()
ax.plot(p_chirps.time,p_gpm.P_mm.mean(dim=['x','y']),color='black',linewidth=0.3,label='GPM IMERG Precipitation')
#ax.plot(p_chirps.time,p_chirps.P_mm.mean(dim=['x','y']),color='C1',label='CHIRPS P')
ax.fill_between(p_chirps.time,0,p_gpm.P_mm.mean(dim=['x','y']), color='black', alpha=0.1) 
...
ax.set_title('Monthly Time Series A',weight='bold',fontsize=15)
plt.tight_layout()
plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\TSA.png',dpi=300)
...
fig = plt.figure(figsize=(8,4))
ax = fig.add_subplot()
ax.plot(et_modis.time,et_modis.ET_kg_m2.mean(dim=['x','y']),color='black',linewidth=0.3,label='ET')
ax.fill_between(et_modis.time,et_modis.ET_kg_m2.mean(dim=['x','y']), color='black', alpha=0.1) 
ax.set_ylim(0,130)
#ax.invert_yaxis()
ax.set_ylabel('Evapotranspiration (kg m{})'.format(get_super('-2')),weight='bold',fontsize=12)
ax.set_xlabel('Date',weight='bold',fontsize=12)
ax.set_xlim(pd.to_datetime('2003-02-01'),pd.to_datetime('2022-07-01'))
plt.legend(loc='upper left')

ax2 = ax.twinx()
ax2.plot(ndvi.time,ndvi.NDVI.mean(dim={'x','y'}),color='darkgreen',label='NDVI',linewidth=1.5) #approximately 1/5 of CLSM
ax2.set_ylim(0.2,0.75)
ax2.set_ylabel('Norm. Difference Vegetation Index',weight='bold',fontsize=11)
plt.legend(loc='upper center')

ax3 = ax.twinx()
ax3.plot(lst.time,lst.LST_K.mean(dim={'x','y'})-273.15,label='LST',color='C3',linewidth=1.5,linestyle='--')
ax3.set_ylabel('Land Surface Temperature ({}C)'.format(degree_sign),weight='bold',fontsize=11)
ax3.set_ylim(5,40)
ax3.spines['right'].set_position(('outward', 40))
plt.legend(loc='upper right')

ax.set_title('Monthly Time Series C',weight='bold',fontsize=15)
plt.tight_layout()
plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\TSC.png',dpi=300)
...
ax.set_title('Monthly Time Series B',weight='bold',fontsize=15)
plt.tight_layout()
plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\TSB.png',dpi=300)





#Mean ET & Soil Moisture
fig = plt.figure(figsize=(8,4))
ax = fig.add_subplot()
ax.plot(et_modis.time,et_modis.ET_kg_m2.mean(dim=['x','y']),color='black',linewidth=0.3,label='ET')
ax.fill_between(et_modis.time,et_modis.ET_kg_m2.mean(dim=['x','y']), color='black', alpha=0.1) 
...
plt.tight_layout()
plt.legend()
plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\runoff_anomalies.png',dpi=300)
...
#plt.plot(spatial_mean_anomalies_vars_monthly[0].time,spatial_mean_anomalies_vars_monthly[0],color='green',label='MODIS ET')
...
#plt.plot(spatial_mean_anomalies_vars_monthly[7].time,spatial_mean_anomalies_vars_monthly[7],color='C0',label='CLSM RZ')
...
plt.xlabel('Date',weight='bold',fontsize=12)
plt.tight_layout()
plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\DroughtIndex_with_VHI_anomalies.png',dpi=300)
...
ax2 = ax.twinx()
ax2.plot(spatial_mean_anomalies_vars_growing[3].year_i,spatial_mean_anomalies_vars_growing[3],color='C2',label='MODIS NDVI')
ax2.plot(spatial_mean_anomalies_vars_growing[5].year_i,spatial_mean_anomalies_vars_growing[5],color='C0',linewidth=2,label='GPM IMERG P')
...
plt.title('Growing Season Anomalies',weight='bold',fontsize=15)
plt.axhline(0,color='black',linestyle='--',linewidth=0.5)
plt.tight_layout()
...
plt.xlim(pd.to_datetime('2003-02-01'),pd.to_datetime('2022-07-01'))
plt.tight_layout()
plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\GrowingSeason_GPM.png',dpi=300)
...
#spatial_mean_anomalies_vars_monthly[5].plot(ax=ax,label='GPM PPT')
spatial_mean_anomalies_vars_growing[5].plot(ax=ax,label='GPM PPT')
plt.xlim(pd.to_datetime('2003-02-01'),pd.to_datetime('2022-07-01'))
plt.ylim(0,250)
...
fig,ax = plt.subplots(figsize=(6,4))
ax2 = ax.twinx()
p_chirps.P_mm.mean(dim={'x','y'}).plot(ax=ax2,linestyle='--',linewidth=0.5,color='C1')
...
def linear_plot(independent,dependent,var1,var2):
    ...
    ax.legend(loc='upper left')
    ax.set_title('{} vs. {}'.format(var1,var2))
    plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\correlations\{}_{}.png'.format(var1,var2))

var_names = ['ET', 'GW','LST','NDVI','CHIRPS P','P','RZ','Q','SMAP SM','SM']

for ii in range(0,len(var_names)):
    for i in range(0,len(var_names)):
        linear_plot(spatial_mean_anomalies_vars_growing[i],spatial_mean_anomalies_vars_growing[ii],var_names[i],var_names[ii])
...
linear_plot(spatial_mean_anomalies_vars_monthly[4][7:-5],spatial_mean_anomalies_indic_monthly[0],'NDVI','VHI')
linear_plot(spatial_mean_anomalies_vars_monthly[4][7:-5],spatial_mean_anomalies_indic_monthly[0],'NDVI','VHI')
linear_plot(spatial_mean_anomalies_vars_monthly[11][:-4],spatial_mean_anomalies_indic_monthly[0],'CLSM RZ','VHI')
...
def MK_pix (dataset):
    dataset = dataset.where(~np.isnan(dataset),0).interpolate_na(dim='year_i')
    y = dataset.y
    x = dataset.x
    '''
    0: trend?
    ...
    '''
    mk_dataset = xr.DataArray(np.apply_along_axis(mk.original_test, axis=0, arr=dataset),
                                dims=('MK_stat','y','x'),
                                coords={'x':x,'y':y})
...
g_mk = MK_pix(var_datasets[1])
g_mk.to_netcdf(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\g_mk_growing_original.nc')
del g_mk
gc.collect()

ndvi_mk = MK_pix(var_datasets[3])
ndvi_mk.to_netcdf(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\ndvi_mk_growing_original.nc')
del ndvi_mk
gc.collect()

ppt_gpm_mk = MK_pix(var_datasets[5])
ppt_gpm_mk.to_netcdf(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\ppt_gpm_mk_growing_original.nc')
del ppt_gpm_mk
gc.collect()

et_mk = MK_pix(var_datasets[0])
et_mk.to_netcdf(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\et_mk_growing_original.nc')
del et_mk
gc.collect()
...
g_mk.to_netcdf(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\g_mk_monthly_seasonal.nc')
del g_mk
gc.collect()

ndvi_mk = MK_pix(var_datasets[4])
ndvi_mk.to_netcdf(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\ndvi_mk_monthly_seasonal.nc')
del ndvi_mk
gc.collect()

ppt_gpm_mk = MK_pix(var_datasets[5])
ppt_gpm_mk.to_netcdf(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\ppt_gpm_mk_monthly_seasonal.nc')
del ppt_gpm_mk
gc.collect()

et_mk = MK_pix(var_datasets[0])
et_mk.to_netcdf(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\et_mk_monthly_seasonal.nc')
del et_mk
gc.collect()
...
#growing season
files_growing = sorted(glob.glob(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\*growing*.nc'))
mk_test_files_growing = [xr.open_mfdataset(file).__xarray_dataarray_variable__ for file in files_growing]
variables = ['ET','GW','NDVI','P']

#Show p-values
for i in range(0,len(mk_test_files_growing)):
    plt.figure(figsize=(100,100))
    mk_test_files_growing[i][2].astype(float).plot(vmax=0.05)
    plt.title('{} p-value'.format(variables[i]))

#Show s-statistic
for i in range(0,len(mk_test_files_growing)):
    plt.figure()
    mk_test_files_growing[i][5].astype(float).plot()
    plt.title('{} S-Statistic'.format(variables[i]))


gw_pval = mk_test_files_growing[1][2].astype(float).rio.write_crs('epsg:4326').rio.clip(limpopo_watershed.geometry.apply(mapping), limpopo_watershed.crs, drop=True,all_touched=False)
gw_stat = mk_test_files_growing[1][5].astype(float).rio.write_crs('epsg:4326').rio.clip(limpopo_watershed.geometry.apply(mapping), limpopo_watershed.crs, drop=True,all_touched=False)

ndvi_pval = mk_test_files_growing[2][2].astype(float).rio.write_crs('epsg:4326').rio.clip(limpopo_watershed.geometry.apply(mapping), limpopo_watershed.crs, drop=True,all_touched=False)
ndvi_stat = mk_test_files_growing[2][5].astype(float).rio.write_crs('epsg:4326').rio.clip(limpopo_watershed.geometry.apply(mapping), limpopo_watershed.crs, drop=True,all_touched=False)

for pval,stat,var in zip([gw_pval,ndvi_pval],[gw_stat,ndvi_stat],['GW','NDVI']):
    pval.rio.to_raster(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\{}_pval.tif'.format(var))
    stat.rio.to_raster(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\{}_stat.tif'.format(var))


et_pval = mk_test_files_growing[0][2].astype(float).rio.write_crs('epsg:4326').rio.clip(limpopo_watershed.geometry.apply(mapping), limpopo_watershed.crs, drop=True,all_touched=False)
et_stat = mk_test_files_growing[0][5].astype(float).rio.write_crs('epsg:4326').rio.clip(limpopo_watershed.geometry.apply(mapping), limpopo_watershed.crs, drop=True,all_touched=False)

p_pval = mk_test_files_growing[3][2].astype(float).rio.write_crs('epsg:4326').rio.clip(limpopo_watershed.geometry.apply(mapping), limpopo_watershed.crs, drop=True,all_touched=False)
p_stat = mk_test_files_growing[3][5].astype(float).rio.write_crs('epsg:4326').rio.clip(limpopo_watershed.geometry.apply(mapping), limpopo_watershed.crs, drop=True,all_touched=False)

for pval,stat,var in zip([et_pval,p_pval],[et_stat,p_stat],['ET','PPT']):
    pval.rio.to_raster(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\{}_pval.tif'.format(var))
    stat.rio.to_raster(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\{}_stat.tif'.format(var))
...
#monthly
#Need to reprocess GW and NDVI because wrong datasets were processed (06-25-23)
files_growing = sorted(glob.glob(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\mk\*monthly*.nc'))
mk_test_files_growing = [xr.open_mfdataset(file).__xarray_dataarray_variable__ for file in files_growing]
variables = ['ET','GW','NDVI','P']
...
for i in range(0,len(mk_test_files_growing)):
    plt.figure()
    mk_test_files_growing[i][2].astype(float).plot(vmax=0.1)
    plt.title('{} p-value'.format(variables[i]))
...
[axB[i].set_title('{}'.format(variable),fontsize=50,fontweight='bold') for i,variable in zip(range(0,3),['SPEI-12','VHI', 'GPM IMERG P'])]
...
plt.tight_layout()
plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\maps\multiplot_16.png',dpi=200)
...
[axB[i].grid(linewidth=0.5) for i in (range(0,3))]
plt.tight_layout()
plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\maps\multiplot_21.png',dpi=200)


i=155+12

smap_da = sm_2016_2021_monthly[2+12][0]
plt.rc('font', size = 40)
fig, (axA, axB) = plt.subplots(2,3,figsize=(45,20))
clm_gw[i].rename('mm').plot(ax=axA[0],cmap='YlGnBu', levels=[i for i in range(400,1000)])
ndvi['NDVI'][i+7].rename(' ').plot(ax=axA[1],cmap='YlGn',levels=[i for i in np.arange(0.25,0.95,0.05)])
...
[axB[i].grid(linewidth=0.5) for i in (range(0,3))]
plt.tight_layout()
plt.savefig(r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\DraftFigures\V2\maps\multiplot_17.png',dpi=200)
