...
dates = pd.date_range('2002-01','2023-01',  freq='1M') 

files = sorted(glob.glob(path+"/MODMYD/*.tif"))
da = [xr.open_rasterio(file)[0].rename(new_name_or_name_dict='ET_kg_m2').rio.write_crs('WGS84').rio.clip(shapefile.geometry.apply(mapping), shapefile.crs, drop=True,all_touched=False)
            for file in files]
dataset = xr.concat(da, dim=dates).rename({'concat_dim':'time'})
dataset.to_netcdf(path+'\MODMYD_ET_2002_2022.nc')


#####################
#2022-10-09

file_paths = r'C:\Users\robin\Box\SouthernAfrica\DATA\PROCESSED\MODIS_ET'
...
for year in years:
    ...
    for month in months:
        ...
        month_mod = xr.open_rasterio(mod[month]).rename(new_name_or_name_dict='ET')
        month_myd = xr.open_rasterio(myd[month]).rename(new_name_or_name_dict='ET')
        y = month_mod.y
        x = month_mod.x
        ...
        savepath = r'C:\Users\robin\Box\SouthernAfrica\DATA\PROCESSED\MODIS_ET\MODMYD'
        array_tif = xr.DataArray(month_modmyd_masked, dims=("y", "x"), coords={"y": y, "x": x}, name="ET")
...
#####################
#08-12-21
#Process ET from downloads

path = r'C:\Users\robin\Desktop\Limpopo_VegetationHydrology\Data\ET_MODIS\Update\2022'
...
mod = xr.open_mfdataset(files[0],parallel=True,chunks={"lat": 1000,"lon":1000}).ET_500m
myd = xr.open_mfdataset(files[1],parallel=True,chunks={"lat": 1000,"lon":1000}).ET_500m

lon = np.array(mod.lon)
lat = np.array(mod.lat)
...
for year in range(2022,2023):
    ...
    mod = xr.open_mfdataset(files[0],parallel=True,chunks={"lat": 1000,"lon":1000}).ET_500m.sel(time='{}'.format(year))
    ...
    array_tifs = [xr.DataArray(month, dims=("lat", "lon"), coords={"lat":lat, "lon":lon}, name="ET_kg_m2")
    ...
    os.chdir(r'C:\Users\robin\Box\SouthernAfrica\DATA\PROCESSED\MODIS_ET\TERRA')
    [array_tif.rio.to_raster('T_MODIS_ET_{}_{:02}.tif'.format(year,month)) for array_tif,month in zip(array_tifs,range(1,13))]

    mod.close()

    #AQUA
    myd = xr.open_mfdataset(files[1],parallel=True,chunks={"lat": 1000,"lon":1000}).ET_500m.sel(time='{}'.format(year))
    ...
    array_tifs = [xr.DataArray(month, dims=("lat", "lon"), coords={"lat":lat, "lon":lon}, name="ET_kg_m2")
                    for month in [jan,feb,mar,apr,may,jun,jul,aug,sep,nov,oct,dec]]
    [array_tif.rio.set_crs("epsg:4326").rio.set_spatial_dims('lon','lat',inplace=True) for array_tif in array_tifs]
    os.chdir(r'C:\Users\robin\Box\SouthernAfrica\DATA\PROCESSED\MODIS_ET\AQUA')
    [array_tif.rio.to_raster('A_MODIS_ET_{}_{:02}.tif'.format(year,month)) for array_tif,month in zip(array_tifs,range(1,13))]
