...
#Data
path_gpm = r'F:\raw-data\GPM_IMERG' #Final Run until 09-30-2021, late run thereafter
path_chirps = r'F:\raw-data\CHIRPS'
...
for year in range(2022,2023):
    ...
    precip_shapefile_monthly_sums = [precip.groupby('time.month').sum(dim={'time'}) for precip in precips_shapefile]


    lats = [precips_shapefile[0].lat,precips_shapefile[1].lat]
    lons = [precips_shapefile[0].lon, precips_shapefile[1].lon]
    data_names = ['CHIRPS', 'GPM_IMERG']

    savepath = r'C:\Users\robin\Box\SouthernAfrica\DATA\PROCESSED\PRECIPITATION'
    array_tifs = [[xr.DataArray(monthly_precip[i], dims=("lat", "lon"), coords={"lat": lat, "lon": lon}, name="P_mm") 
                  for monthly_precip,lat,lon, in zip(precip_shapefile_monthly_sums,lats,lons)] for i in range(0,12)]

    [[array_tif[i].rio.set_crs("epsg:4326").rio.set_spatial_dims('lon','lat',inplace=True) for array_tif in array_tifs] for i in range(0,2)] 
    [[array_tif[i].rio.to_raster(savepath+'/{}/{}_{:02}.tif'.format(data,year,month+1)) for array_tif,month in zip(array_tifs,range(0,12))] for i,data in zip(range(0,2),data_names)] 
