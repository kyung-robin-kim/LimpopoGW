...
#(-2* gw_anomaly.std(axis=1,ddof=1) + gw_anomaly.mean(axis=1)).plot()
#(2* gw_anomaly.std(axis=1,ddof=1) + gw_anomaly.mean(axis=1)).plot()
monthly_anomalies_q.mean(axis=1).rename('Discharge').plot(legend=True)
monthly_anomalies_gw.mean(axis=1).rename('GW Level').plot(legend=True)
plt.xlabel('Month')
plt.ylabel('Anomaly')
...
#Create Monthly Anomalies Datasets (normalized)
'''
#LST, NDVI, GW, SM
...
variables = [var[(var.time>pd.to_datetime('2003-02')) & (var.time<pd.to_datetime('2022-01'))] for var in variables]
...
variables = [et_modis.ET_kg_m2, pet_modis.ET_kg_m2]
[print(var.dims) for var in variables]
...
precip_variables = [var[(var.time>pd.to_datetime('2003-02')) & (var.time<pd.to_datetime('2022-01'))] for var in precip_variables]
...
#vhi_dataset = [xr.open_mfdataset(file,parallel=True) for file in files_vhi]
#monthly_anom(vhi_dataset[0].VHI.rio.write_crs('epsg:4326').rio.reproject_match(ndvi.rio.write_crs('epsg:4326'), resampling=rio.enums.Resampling.bilinear)).to_netcdf(netcdf_anom_path+r'\VHI\VHI_anom.nc')
